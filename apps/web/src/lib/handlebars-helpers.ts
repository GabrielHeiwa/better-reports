...
export function registerHelpers() {
  // Number formatting: {{format value "0,0.00"}} or {{format value}}
  ...
  // Currency: {{currency value}} → R$ 1.234,56
  ...
  // Date: {{date value "dd/MM/yyyy"}} — value can be ISO string or timestamp
  ...
  // Comparison (for use inside {{#if (eq a b)}})
  ...
  Handlebars.registerHelper('sum', (arr: unknown, key?: unknown) => {
    ...
    return arr.reduce((acc, item) => acc + Number(typeof key === 'string' ? item[key] : item), 0)
  })
}
